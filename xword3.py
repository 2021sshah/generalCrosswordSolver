# Siddharth Shah Gabor Pd. 2 from xword2d
...
def possibleChoices(mainBoard): # Brunt Work of Ordering Moves
    ...
    # First Is Heuristic
    newList = [(idxToMoveWeight[idx], idxToChangedSet[idx]) for idx in idxToChangedSet if len(idxToChangedSet[idx])]
    toRet = sortWeights(newList)
    return toRet

def sortWeights(weightList):
    bucket = [[] for i in range(maxLEN)]
    for tup in weightList:
        bucket[tup[0]].append(tup)
    return [tup for sub in bucket for tup in sub][::-1]

def moveWeight(board, row, col, flippedList): # Smaller is Better
    up = down = left = right = 0 # Distance To Block In 4 Directions
    ...
    for r in range(row+1, HEIGHT): # Down
        if board[r][col] == blockCHAR: break
        down += 1
    return (up+down+left+right) + (up*down) + (right*left)
    #return int(len(flippedList)*100 - (up+down+left+right) - (up*down) - (right*left))
...
board = placeAllWords(board, wordsToAdd.copy(), set(), 1)
showBoard(board)
print("Total Time: {}s".format(round(time.time()-startTime,3)))